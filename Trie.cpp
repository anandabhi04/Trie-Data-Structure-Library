...
class TrieDS{
   ...
   int stringToTrie(trieNode *node, string &serialTrie, int &read)
   {
    ...
    void autoSuggest(string word){
        ...
        //prints list of word with given prefix;
        //problem- don't work realtime and is not user friendly;
        helpAuto(word,curr);

    }

    //Function to delete a word from the trie
    //if prefixCount becomes 0 delete the node;
...
int main(){
    ...
    // Taken input from this file
    freopen ("new_output.txt", "w", stdout);                                   
     // Given output to this file
    ...
    vector<string> input;
 	...
 	cin>>word;
 	while(word != "#END#")                                              
    //NOTE-PASS #END# TO GET OUT OF THE LOOP
    {
    ...
    // Make a trie object using the words provided by user
 	cout<<"Your serialised String is:-"<<"   "<<myTrie->trieToString()<<endl;               
    // Converted a trie into a string  (Serialisation)
    cout<<"Total words in a Trie is:-"<<"   "<<myTrie->numberOfWordsInTrie()<<endl;         
    //Counted the number of words in a trie

    cout<<"Enter some words to insert in this trie"<<endl;
    cin>>word;
 	while(word != "#END#")
    {
       myTrie->insertAtTrie(word);                                                             
       // Inserted a word to the trie
       cin>>word;
    }
    
    string serialstr=myTrie->trieToString();                                                  
     // Coverted the trie into a string (Serialisation)
    cout<<"Your new serialised String is:-"<<"   "<<serialstr<<endl;
    
    cout<<"Enter a word to search in a trie:-   ";
    cin>>word;
    if(myTrie->searchInTrie(word)){                                                             
    ...
    TrieDS *myTrie2 = new TrieDS(serialstr);                                               
     //Made a new Trie using the Serialised String of the old Trie
    vector<string> v2= myTrie2->trieToList();                                              
     //Make a vector with the words of new Trie
    cout<<"New Trie:-   ";
    for(auto a:v2){
        cout<<a<<" ";
    }
    ...
    cin>>word;
    myTrie2->deleteInTrie(word);                                                                
    //Delete a number from the New Trie
    v2= myTrie2->trieToList();                                                                  
    //Make a vector with the words of new Trie
    ...
    cin>>word;
    cout<<"Total number of words with prefix '"<<word<<"' are:-   "<<myTrie2->wordsWithThePrefix(word)<<endl;           
    //Find the total number of words with prefix
    ...
    cin>>word;
    cout<<"The words are:-   ";
    myTrie2->autoSuggest(word);                                                                         
    //Suggest the words with a given Prefix
